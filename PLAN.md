...
- [x] `kvm` advertised unconditionally, nesting off by default *(2026-09-20)*:
      ...
      to TCG without `/dev/kvm`, as linux-builder always did. Nested KVM
      stalled the image build; the 09-22 reproduction found intermittent
      nested guest faults, superseding the original performance diagnosis
      (D22). Both hosts build it with nesting disabled.
